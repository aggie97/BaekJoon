/*문제
  ...
  영석이는 칠판에 글자들을 수평으로 일렬로 붙여서 단어를 만든다. 다시 그 아래쪽에 글자들을 붙여서
  또 다른 언어를 만든다. 이런 식으로 다섯 개의 단어를 만든다. 아래 그림 1은 영석이가 칠판에 붙여 만든 단어들의 예이다.
  ...
  한 줄의 단어는 글자들을 빈칸 없이 연속으로 나열해서 최대 15개의 글자들로 이루어진다. 또한
  ...
  위의 그림 1의 다섯 번째 자리를 보면 두 번째 줄읠 다섯 번째 자리의 글자는 없다. 이런 경우처럼
...
주어지는 글자는 영어 대문자 'A'부터 'Z', 영어 소문자 'a'부터 'z', 숫자 '0'부터 '9'중 하나 이다.
...
let answer = "";
const colCountLength = Math.max(...input.map((v) => v.length));
